...
- `github_get_issue_handler`: Get detailed issue information including fields, assignees, dates
...
2. Use `github_get_issue_handler` via BOSA MCP to fetch detailed issue data 
3. Use `github_list_issues_comments` via BOSA MCP to get comments for R7 evaluation
4. Use `github_merge_tool` to consolidate all retrieved data
5. Use `github_compliance_evaluator_tool` to evaluate against 7 rules
6. Use `github_report_generator_tool` to create final compliance report
7. Use `github_formatter_tool` for final output formatting if needed
...
class AgentCreator:
    ...
    def create_complete_setup(self, config_file: Optional[str] = None) -> bool:
        ...
        self.logger.info(f"4. List tools: aip tools list")
